...
$app->hook('cards', function ($param) use ($use) {
    ...
    $query = "
            select
                DISTINCT c.card_id, GROUP_CONCAT(g.group_id SEPARATOR ', ') group_id,
            ...
            where
                ...
                u.user_id='".$use->users["user_id"]."' and
...
$app->hook('pagging', function ($param) use ($use) {
    ...
    if($select->execute()){
        ...
        if($page > 1){
            $data_mst["prev"]=array(
                    "url"=>$use->app->urlFor('home', array("page"=>1)),
                    "title"=>1
            );
        }
        for($i=1;$i<(ceil($count/$limit)+1);$i++){
            $thispagging = array(
                "url"=>$use->app->urlFor('home', array("page"=>$i)),
                "title"=>$i,
        ...
        if($page != $a){
            $data_mst["next"]=array(
                    "url"=>$use->app->urlFor('home', array("page"=>$a)),
                    "title"=>$a
            );
        }else{
           $data_mst["next"]=false;
...
$app->hook('hint', function ($param) use ($use) {
    ...
    $company_hint = function() use ($use){
        $select = $use->db->prepare("select co.company_name from users_groups u, groups g, cards_groups cg, cards c, cards_company co where u.group_id=g.group_id AND g.group_id=cg.group_id and cg.card_id=c.card_id and c.company_id=co.company_id and u.user_id=:user_id group by co.company_name order by co.company_name");
    ...
    $occupation_hint = function() use ($use){
        $select = $use->db->prepare("select o.occupation_name from users_groups u, groups g, cards_groups cg, cards c, cards_occupations co, occupations o WHERE u.group_id=g.group_id AND g.group_id=cg.group_id and cg.card_id=c.card_id and c.card_id=co.card_id and co.occupation_id=o.occupation_id and u.user_id=:user_id group by o.occupation_name order by o.occupation_name");
        $select->bindParam(':user_id', $use->users["user_id"]);
        $select->execute();
        $result = array();
        foreach($select->fetchAll(PDO::FETCH_NUM) as $row){
    ...
    $country_hint = function() use ($use){
        $result = array();
        $select = $use->db->prepare("SELECT co.country_name, co.country_name_ind FROM  users_groups u, groups g, cards_groups cg, cards c, cards_city ci, cards_country co WHERE u.group_id=g.group_id AND g.group_id=cg.group_id and cg.card_id=c.card_id and c.city_id=ci.city_id AND ci.country_id=co.country_id and u.user_id=:user_id group by ci.city_name order by co.country_name");
        $select->bindParam(':user_id', $use->users["user_id"]);
        $select->execute();
        foreach($select->fetchAll(PDO::FETCH_NUM) as $row){
            array_push($result, $row[0]);
...
$app->get('/list/company/:page', function ($page) use ($use) {
    ...
    $limit = 9;
    $start = $limit*($page-1);
    $select = $use->db->prepare("select co.company_name from users_groups u, groups g, cards_groups cg, cards c, cards_company co where u.group_id=g.group_id AND g.group_id=cg.group_id and cg.card_id=c.card_id and c.company_id=co.company_id and u.user_id=:user_id group by co.company_name limit ".$start.", ".$limit);
    $select->bindParam(':user_id', $use->users["user_id"]);
    if($select->execute()){
        $result = array();
        foreach($select->fetchAll(PDO::FETCH_NUM) as $row){
...
$app->get('/list/country/:page', function ($page) use ($use) {
    ...
    $limit = 9;
    $start = $limit*($page-1);
    $select = $use->db->prepare("SELECT co.country_name FROM  users_groups u, groups g, cards_groups cg, cards c, cards_city ci, cards_country co WHERE u.group_id=g.group_id AND  g.group_id=cg.group_id and cg.card_id=c.card_id and c.city_id=ci.city_id AND ci.country_id=co.country_id and u.user_id=:user_id group by co.country_name  limit ".$start.", ".$limit);
    $select->bindParam(':user_id', $use->users["user_id"]);
    if($select->execute()){
        $result = array();
        foreach($select->fetchAll(PDO::FETCH_NUM) as $row){
...
$app->get('/list/category/:page', function ($page) use ($use) {
    ...
    $limit = 9;
    $start = $limit*($page-1);
    $select = $use->db->prepare("SELECT ca.category_name FROM  users_groups u, groups g, cards_groups cg, cards c, cards_categories cc, categories ca WHERE u.group_id=g.group_id AND  g.group_id=cg.group_id and cg.card_id=c.card_id and c.card_id=cc.card_id and cc.category_id=ca.category_id and u.user_id=:user_id group by ca.category_name  limit ".$start.", ".$limit);
    $select->bindParam(':user_id', $use->users["user_id"]);
    if($select->execute()){
        $result = array();
        foreach($select->fetchAll(PDO::FETCH_NUM) as $row){
